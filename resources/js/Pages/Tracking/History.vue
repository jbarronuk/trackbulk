...
const pollTrackingUpdates = () => {
  intervalId = setInterval(() => {
    axios.get('/api/tracking?format=date&start=' + new Date(0).toISOString() + '&end=' + new Date(new Date().setHours(23, 59, 59, 999)).toISOString())
      .then(response => {
...
const changed = () => {
  const setA = [...Object.values(selected.value)].sort();
  const setB = trackingData.value.flatMap(batch => batch.tracking.map(track => track.id)).sort();
    ...
    <table class="min-w-full border divide-y divide-gray-200 dark:divide-gray-700">
      ...
      <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
        <tr v-if="batches.length == 0">
            <td class="px-3 text-center text-sm text-gray-600 dark:text-gray-300" colspan="4">No history available, <a class="underline" :href="route('tracking.index')">start tracking</a></td>
        </tr>
        <template v-for="(batch, batchIndex) in trackingData" :key="batch.id">
          <tr>
            <td @click="selectBatch(batch)" class="hover:cursor-pointer px-3 text-sm text-gray-600 dark:text-gray-300" colspan="4">Batch: {{ batch.formatted_created_at }}</td>
          </tr>
          <tr v-for="(track, trackingIndex) in batch.tracking" :key="track.id">
            ...
            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-200 bg-white dark:bg-gray-900 text-right">
              <input :id="'track_id_' + track.id" :value="track.id" v-model="selected" @change="changed" type="checkbox" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            </td>
          </tr>
        </template>
      </tbody>
